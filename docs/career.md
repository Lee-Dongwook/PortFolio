...
| §3 AI Native 역량 디테일 | [-] | 챗 골격 완료: `entities/chat/{model,config}` (ChatNode/Script/Zustand store) + `widgets/chat-bubble` (플로팅 우하단, 패널 토글, 메시지 + 칩, reset/close). `app/[locale]/layout.tsx` 에 ChatBubble 글로벌 마운트. 모든 노드 텍스트는 placeholder "테스트 내용입니다" — 실 콘텐츠(career.md §1·§2·§3·§5·§6·§7 매핑) 및 i18n/attachment 정책은 별도 청크. revamp-plan S1 결정형 챗 기준. |
